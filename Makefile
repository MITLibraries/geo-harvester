...
.PHONY: help install venv update test coveralls lint lint-fix security harvester check-arch dist-local dist-dev publish-dev docker-clean dist-stage publish-stage
...
dist-local: check-arch # Build docker container locally without publishing
...
publish-dev: dist-dev # Build, tag and push (intended for developer-based manual publish)
	@ARCH_TAG=$$(cat .arch_tag); \
	aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin $$(echo $(ECR_URL_DEV) | cut -d'/' -f1); \
